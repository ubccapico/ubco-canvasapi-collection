...
def revel():
    ...
    try:
        ...
        title = input("\nEnsure the Revel XLSX file is in the same location as this script. Enter the name of the file (including the extension .xlsx) and then press Enter: ").strip()
        try:
            if title=='quit': #If users want to quit, exit program
                return
            originalFile = pandas.read_excel(title, skiprows=1)
            try:
                #Start processing the file
                #Combine first and last name
                originalFile["Student"] = originalFile["First Name"].map(str) + " " + originalFile["Last Name"]
                originalFile["Student"] = originalFile["Student"].str.title()
...
def getFileName():
    ...
    text_String = "\nMake sure the exported spreadsheet XLSX file is in the same directory as this script. Enter the name of the file, including the extension (.xlsx), and then press Enter (or enter quit to exit): "
    title = input(text_String).strip()
    path = title
    textError = ''
    #While the file does not exist, isn't a csv, or the user wants to quit
    while(len(glob.glob(path))!=1 or not title.endswith('.xlsx') or title == 'quit'):
